...
// internalNumber = 30 상위 스코프에서 하위 스코프 멤버에 접근할 수 있다
// 하위 -> 상위로 접근은 가능
...
class Numbers(var number: Int = 10){
    fun changeNumber(){
        this.number = 100 // Number 라는 스코프 안에 존재하기 때문에 number에 접근/변경 할 수 있다
    }
    fun whatIsNumber(){
        return this.number
    }
